...
const centerTable = (results) => {
  ...
  const tableString = consoleTable.getTable(results);
   // Calc the width of the table
  const tableWidth = tableString.split("\n")[0].length;
    // Calc the number of spaces needed for centering
  const spaces = Math.floor((process.stdout.columns - tableWidth) / 2);
   // Add space around the entire table
...
const viewEmByManager = (m_id) => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT e.id, CONCAT(e.first_name, ' ', e.last_name) AS name, CONCAT(m.first_name, ' ', m.last_name) AS manager,
...
const addDepartment = (department) => {
  return new Promise((resolve, reject) => {
    db.query("INSERT INTO department SET ?", department, (err, results) => {
      if (err) reject(err);
      console.log(`Added department: ${department.name} successfully!`); // works
      resolve(results);
    });
  });
};
const addRole = (role) => {
  return new Promise((resolve, reject) => {
    db.query("INSERT INTO role SET ?", role, (err, results) => {
      if (err) reject(err);
      console.log(`Added role: ${role.title} successfully!`); // works
      resolve(results);
    });
  });
};
